...
class ShareDummyVecEnv(ShareVecEnv):
    def __init__(self, env_fns):
        ...
        env = self.envs[0]
        ShareVecEnv.__init__(self, len(
            env_fns), env.observation_space, env.share_observation_space, 
                      env.action_space)
        self.actions = None
    ...
    def step_wait(self):
        results = [env.step(a) for (a,env) in zip(self.actions, self.envs)]
        # print(len(*results))
        obs, agent_state, rews, dones,  \
        infos, available_actions = map(
            np.array, zip(*results))
        
        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    obs[i], agent_state[i], available_actions[i] = self.envs[i].reset()
            else:
                if np.all(done):
                    # print("state_size: ", np.shape(agent_state))
                    obs[i], agent_state[i], available_actions[i] = self.envs[i].reset()
        self.actions = None

        return obs, agent_state, rews, dones, infos, available_actions

    def reset(self):
        # print(self.envs)
        results = [env.reset() for env in self.envs]
        obs, agent_state, available_actions = map(np.array, zip(*results))
        return obs, agent_state, available_actions
...
class ShareMultiDummyVecEnv(ShareMultiVecEnv):
    def __init__(self, env_fns):
        self.envs = [fn() for fn in env_fns]
        env = self.envs[0]
        ShareMultiVecEnv.__init__(self, len(
