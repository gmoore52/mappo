...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.add_local_obs = True
        self.add_move_state = True
        self.add_visible_state = True
        self.add_distance_state = True
        self.add_xy_state = True
        self.add_enemy_action_state = True
        self.add_agent_id = True
        self.use_state_agent = True
        self.use_mustalive = False
        self.add_center_xy = True
        self.use_stacked_frames = False
        self.closest_attack = False
        ...
        self.mask_agents = "agent_mask" in self.capability_config
        ...
        self.prob_obs_agent = prob_obs_agent
        ...
        self.enemy_attack_probabilities = np.zeros(self.n_enemies)
        self.agent_health_levels = np.zeros(self.n_agents)
        self.enemy_health_levels = np.zeros(self.n_enemies)
        self.death_tracker_ally = np.zeros(self.n_agents)
        self.death_tracker_enemy = np.zeros(self.n_enemies)
        self.fov_directions_agent = np.zeros((self.n_agents, 2))
        self.fov_directions_agent[:, 0] = 1.0
        self.fov_directions_enemy = np.zeros((self.n_enemies, 2))
        self.fov_directions_enemy[:, 0] = 1.0
        ...
        self.new_enemy_positions = np.zeros((self.n_enemies, 2))
        ...
        self._interface_options = [sc_pb.InterfaceOptions(raw=True, score=False) for _ in range(self._n_players)]
    ...
    def _launch(self):
        ...
        # Setup controllers for each process
        ...
        # Use primary agent controller to create/host game
        self._controllers[self.agent_index].create_game(create)
        
        joinReqs = []
        # Create join requests for the controllers
        for i in range(self._n_players):
            join = sc_pb.RequestJoinGame(options=self._interface_options[i],
                                         race=races[self._agent_race if i == self.agent_index
                                                    else self._bot_race])
            join.shared_port = 0
    ...
    def reset(self, episode_config={}):
        ...
        self.enemy_attack_probabilities = episode_config.get("attack", {}).get(
            "item", None
        )
        self.agent_health_levels = episode_config.get("health", {}).get(
            "item", None
        )
        self.enemy_health_levels = episode_config.get("health", {}).get(
            "item", None
        )
        self.enemy_mask = episode_config.get("enemy_mask", {}).get(
            "item", None
        )
        self.agent_mask = episode_config.get("agent_mask", {}.get(
            "item", None
        ))
        ...
        self.agent_tags = [None for i in range(self.n_agents)]
        self.death_tracker_ally = np.zeros(self.n_agents)
        self.death_tracker_enemy = np.zeros(self.n_enemies)
        self.fov_directions_agent = np.zeros((self.n_agents, 2))
        self.fov_directions_agent[:, 0] = 1.0
        ...
        self.last_action = np.zeros((self.n_agents, self.n_agent_actions))
        self.last_action_enemy = np.zeros((self.n_enemies, self.n_enemy_actions))
        ...
        try:
            ...
            # logging.info("Full restart required in reset()")
            self.full_restart()
        ...
        if self.use_state_agent:
            agent_global_state = [self.get_state_agent(a_id) for a_id in range(self.n_agents)]
            enemy_global_state = [self.get_state_enemy(e_id) for e_id in range(self.n_enemies)]
        else:
            agent_global_state = [self.get_state(agent_id) for agent_id in range(self.n_agents)]
            enemy_global_state = [self.get_global_state_enemy(e_id) for e_id in range(self.n_enemies)]
        ...
        return  local_obs, enemy_obs, \
                agent_global_state, enemy_global_state,\
                avail_actions, avail_actions_enemy
    ...
    def _restart(self):
        ...
        try:
            ...
            # logging.info("Full restart required in restart()")
            self.full_restart()
    ...
    def step(self, agent_actions, enemy_actions):
        ...
        self.last_action = np.eye(self.n_agent_actions)[np.array(agent_actions_int)]
        self.last_action_enemy = np.eye(self.n_enemy_actions)[np.array(enemy_actions_int)]
        ...
        for a_id, action in enumerate(agent_actions_int):
        ...
        for e_id, action in enumerate(enemy_actions_int):
            if not self.heuristic_ai:
                sc_action = self.get_enemy_action(e_id, action)
            else:
                sc_action, action_num = self.get_enemy_action_heuristic(
                    e_id, action
                )
        ...
        sc_enemy_actions = [[action] for action in sc_enemy_actions]
        
        action_touples_agent = [(self._controllers[self.agent_index], action) for action in sc_agent_actions]
        action_touples_enemy = [(self._controllers[0 if self.agent_index==1 else 1], action) for action in sc_enemy_actions]
        ...
        try:
            ...
            if not self.stochastic_health:
                ...
                self._parallel.run((c.step, self._step_mul-self._kill_unit_step_mul)
                                   for c in self._controllers)
                self._kill_units_below_health_level()
                self._parallel.run((c.step, self._kill_unit_step_mul) for c in self._controllers)
            # Observe here so that we know if the episode is over.
            self._obs = self._controllers[self.agent_index].observe()
        except (protocol.ProtocolError, protocol.ConnectionError):
            # logging.info(f"Full restart required in step()\nDue to {e}")
            self.full_restart()
            terminated = True
            avail_actions = []
            avail_enemy_actions = []
            ...
            for i in range(self.n_agents):
                ...
                if terminated:
                    ...
                    if self.death_tracker_ally[i]:
                        ...
                        dones[i] = False
            for i in range(self.n_enemies):
                avail_enemy_actions.append(self.get_avail_enemy_actions(i))
                enemy_infos[i] = {
            ...
            if self.use_state_agent:
                ...
                agent_global_state = [self.get_state(a_id) for a_id in range(self.n_agents)]
                enemy_global_state = [self.get_global_state_enemy(e_id) for e_id in range(self.n_enemies)]    
            ...
            return local_obs, enemy_obs, agent_global_state, enemy_global_state, \
                    ...
                    infos, enemy_infos, avail_actions, avail_enemy_actions
        ...
        if self.use_state_agent:
            global_state = [self.get_state_agent(a_id) for a_id in range(self.n_agents)]
            enemy_global_state = [self.get_state_enemy(e_id) for e_id in range(self.n_enemies)]
        else:
            global_state = [self.get_state(a_id) for a_id in range(self.n_agents)]        
            enemy_global_state = [self.get_global_state_enemy(e_id) for e_id in range(self.n_enemies)]
        ...
        return local_obs, enemy_obs, global_state, enemy_global_state, rewards, enemy_rewards, \
               dones, enemy_dones, infos, enemy_infos, avail_actions, avail_enemy_actions
    ...
    def get_agent_action(self, a_id, action):
        ...
        if action == 0:
            ...
            self.fov_directions_agent[a_id] = self.canonical_fov_directions[
                action - 6
            ]
            cmd = None
            ...
            can_shoot = self.get_agent_can_shoot(a_id, target_unit)
    ...
    def get_enemy_action(self, e_id, action):
        """Construct the action for Enemy e_id."""
        avail_actions = self.get_avail_enemy_actions(e_id)
        assert (
            avail_actions[action] == 1
        ), "Enemy {} cannot perform action {}".format(e_id, action)

        unit = self.get_enemy_by_id(e_id)
        tag = unit.tag
        x = unit.pos.x
        y = unit.pos.y
        ...
        if action == 0:
            ...
            # move north for agent, south for enemy
            cmd = r_pb.ActionRawUnitCommand(
                ability_id=actions["move"],
                target_world_space_pos=sc_common.Point2D(
                    x=x, y=y - self._move_amount
            ...
            self.new_enemy_positions[e_id] = np.array(
                [x, y - self._move_amount]
            )
            if self.debug:
            ...
            # move south for agent, north for enemy
            cmd = r_pb.ActionRawUnitCommand(
                ability_id=actions["move"],
                target_world_space_pos=sc_common.Point2D(
                    x=x, y=y + self._move_amount
            ...
            # move east for agent, west for enemy
            cmd = r_pb.ActionRawUnitCommand(
                ability_id=actions["move"],
                target_world_space_pos=sc_common.Point2D(
                    x=x - self._move_amount, y=y
            ...
            self.fov_directions_enemy[e_id] = self.canonical_fov_directions[
                action - 6
            ]
            cmd = None
            ...
            if self.stochastic_attack:
                p = np.random.default_rng().uniform()
                if p > self.enemy_attack_probabilities[e_id]:
                    if self.debug:
                        logging.debug(
                            f"Enemy {e_id} {action_name}s {target_id}, but fails"
                        )
                    return None
            action_id = actions[action_name]
            target_tag = target_unit.tag

            can_shoot = self.get_enemy_can_shoot(e_id, target_unit)
    ...
    def reward_battle(self):
        ...
        if self.reward_only_positive:
            ...
            reward_enemy = max(delta_enemy_enemy + delta_deaths_enemy, 0)
        else:
    ...
    def enemy_shoot_range(self, enemy_id):
        """Returns the shooting range for an agent."""
        if self.use_unit_ranges:
            attack_range_map = {
                self.stalker_id: 6,
    ...
    def enemy_sight_range(self, enemy_id):
        """Returns the sight range for an agent."""
        # get the unit
        if self.use_unit_ranges:
            sight_range_map = {
            ...
            if self.obs_last_action:
                feature_names.extend(
                    [
                        ...
                        for action in range(self.n_agent_actions)
                    ]
                )
    ...
    def _compute_health_agent(self, agent_id, unit):
    ...
    def _compute_health_enemy(self, enemy_id, unit):
        """Each agent has a health bar with max health
        `health_max` and current health `health`. We set a level
        `health_level` between `0` and `1` where the agent dies if its
        proportional health (`health / health_max`) is below that level.
        ...
        """
        proportional_health = unit.health / unit.health_max
        health_level = self.enemy_health_levels[enemy_id]
        return (1.0 / (1 - health_level)) * (
            proportional_health - health_level
        )
    ...
    def render_fovs(self):
        ...
        self._controllers[self.agent_index].debug(debug_command)

    def agent_cone(self, agent_id):
        fov_direction = self.fov_directions_agent[agent_id]
    ...
    def enemy_cone(self, enemy_id):
        fov_direction = self.fov_directions_enemy[enemy_id]
        c, s = np.cos(self.conic_fov_angle / 2), np.sin(
            self.conic_fov_angle / 2
        )
        sight_range = self.enemy_sight_range(enemy_id)
        rot = np.array([[c, -s], [s, c]])
        neg_rot = np.array([[c, s], [-s, c]])
        start_pos = self.new_unit_positions[enemy_id]
        init_pos = sc_common.Point(
            x=start_pos[0],
            y=start_pos[1],
            z=self.get_enemy_by_id(enemy_id).pos.z,
        )
        upper_cone_end = start_pos + (rot @ fov_direction) * sight_range
        lower_cone_end = start_pos + (neg_rot @ fov_direction) * sight_range
        lines = [
    ...
    def is_position_in_unit_cone(self, agent_id, pos, range="sight_range"):
        ...
        x = self.fov_directions_agent[agent_id][0]
        x = max(x, EPS) if x_diff > 0 else min(x, -EPS)
        fov_angle = np.arctan(self.fov_directions_agent[agent_id][1] / x)
        return np.abs(obj_angle - fov_angle) < self.conic_fov_angle / 2

    def is_position_in_enemy_cone(self, enemy_id, pos, range="sight_range"):
        ally_pos = self.get_enemy_by_id(enemy_id).pos
        distance = self.distance(ally_pos.x, ally_pos.y, pos.x, pos.y)
        # position is in this agent's cone if it is not outside the sight
        # range and has the correct angle
        if range == "sight_range":
            unit_range = self.enemy_sight_range(enemy_id)
        elif range == "shoot_range":
            unit_range = self.enemy_shoot_range(enemy_id)
        else:
            raise Exception("Range argument not recognised")
        if distance > unit_range:
            return False
        ...
        x_diff = max(x_diff, EPS) if x_diff > 0 else min(x_diff, -EPS)
        obj_angle = np.arctan((pos.y - ally_pos.y) / x_diff)
        x = self.fov_directions_enemy[enemy_id][0]
        x = max(x, EPS) if x_diff > 0 else min(x, -EPS)
        fov_angle = np.arctan(self.fov_directions_enemy[enemy_id][1] / x)
        return np.abs(obj_angle - fov_angle) < self.conic_fov_angle / 2
    ...
    def get_obs_agent(self, agent_id, fully_observable=False):
        ...
        if (
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                enemy_visible = (
                    self.is_position_in_unit_cone(agent_id, e_unit.pos)
                    if self.conic_fov
            ...
            for i, al_id in enumerate(al_ids):
                ...
                ally_visible = (
                    self.is_position_in_unit_cone(agent_id, al_unit.pos)
                    if self.conic_fov
                    else dist < sight_range
                )
                if (ally_visible and al_unit.health > 0) or (
                    ...
                    if self.obs_all_health:
                        if not self.stochastic_health:
                            ...
                            ally_feats[i, ind] = self._compute_health_agent(
            ...
            if self.obs_own_health:
                if not self.stochastic_health:
                    ...
                    own_feats[ind] = self._compute_health_agent(agent_id, unit)
                ind += 1
                if self.shield_bits_ally > 0:
            ...
            if self.obs_own_pos:
                ...
                ind += 2
            if self.conic_fov:
                own_feats[ind : ind + 2] = self.fov_directions_agent[agent_id]
                ind += 2
    ...
    def get_obs_enemy(self, enemy_id, fully_observable=False):
        ...
        if (
            ...
            true_avail_actions = self.get_true_avail_enemy_actions(enemy_id)
            for m in range(self.n_actions_move):
                move_feats[m] = avail_actions[m + 2]
            ...
            for e_id, e_unit in self.agents.items():
                ...
                dist = self.distance(x, y, e_x, e_y)
                enemy_visible = (
                    self.is_position_in_enemy_cone(enemy_id, e_unit.pos)
                    if self.conic_fov
                    else dist < sight_range
                )
                if enemy_visible:
                    if self.agent_tags[e_id] is None:
                        self.obs_agents[e_id, enemy_id] = 1
                        self.agent_tags[e_id] = enemy_id
                        for a_id in range(self.n_enemies):
                            if a_id != enemy_id:
                                draw = np.random.rand()
                                if draw < self.prob_obs_agent:
                                    self.obs_agents[e_id, a_id] = 1
                ...
                if (enemy_visible and e_unit.health > 0) or (
                    ...
                    show_enemy = (
                        self.mask_agents
                        and not self.agent_mask[enemy_id][e_id]
                    ) or not self.mask_agents
                    ind = 4
                    if self.obs_all_health and show_enemy:
                        enemy_feats[e_id, ind] = (
                            e_unit.health / e_unit.health_max
            ...
            for i, al_id in enumerate(al_ids):
                ...
                dist = self.distance(x, y, al_x, al_y)
                ally_visible = (
                    self.is_position_in_enemy_cone(enemy_id, al_unit.pos)
                    if self.conic_fov
                    else dist < sight_range
                )
                if (ally_visible and al_unit.health > 0) or (
                    ...
                    if self.obs_all_health:
                        if not self.stochastic_health:
                            ...
                            ally_feats[i, ind] = self._compute_health_enemy(
                                enemy_id=al_id, unit=al_unit
                            )
                            ind += 1
                        elif self.zero_pad_health:
                            ind += 1
                        if self.shield_bits_ally > 0:
                            ...
                            ind += 1
                    if self.stochastic_attack and self.observe_attack_probs:
                        ally_feats[i, ind] = self.enemy_attack_probabilities[
                            al_id
                        ]
                        ind += 1
                    elif (
                    ...
                    if self.stochastic_health and self.observe_teammate_health:
                        ally_feats[i, ind] = self.enemy_health_levels[al_id]
                        ind += 1
                    elif self.stochastic_health and self.zero_pad_health:
                        ind += 1
                    if self.unit_type_bits > 0 and (
            ...
            if self.stochastic_attack:
                own_feats[ind] = self.enemy_attack_probabilities[enemy_id]
                ind += 1
            if self.stochastic_health:
                own_feats[ind] = self.enemy_health_levels[enemy_id]
                ind += 1
            if self.obs_own_pos:
                own_feats[ind] = x / self.map_x
                own_feats[ind + 1] = y / self.map_y
                ind += 2
            if self.conic_fov:
                own_feats[ind : ind + 2] = self.fov_directions_enemy[enemy_id]
                ind += 2
            # own_feats[ind] = self.unit_shoot_range(enemy_id)
            # ind += 1
            if self.unit_type_bits > 0:
        ...
        if self.debug:
            ...
            logging.debug(
                "Avail. actions {}".format(
                    self.get_avail_agent_actions(enemy_id)
                )
            )
            logging.debug("Move feats {}".format(move_feats))
            logging.debug("Enemy feats {}".format(enemy_feats))
    ...
    def get_enemy_obs(self):
        enemy_obs = [self.get_obs_enemy(i) for i in range(self.n_enemies)]
        return enemy_obs
        ...
        if (self.use_mustalive and unit.health > 0) or (not self.use_mustalive):
            ...
            for al_id, al_unit in self.agents.items():
                if al_unit.health > 0:
                    ...
                    if self.add_center_xy:
                        ally_state[al_id, ind] = (al_x - center_x) / self.max_distance_x
                    ...
                    if unit.health > 0:
                        ind += self.unit_type_bits
                        if self.add_distance_state:
                            ally_state[al_id, ind] = dist / sight_range
                            ind += 1
                        if self.add_xy_state:
                            ally_state[al_id, ind] = (al_x - x) / sight_range
    ...
    def get_state_enemy(self, enemy_id):
        """Returns observation for enemy_id. The observation is composed of:
        ...
        move_feats_dim = self.get_obs_move_feats_size()
        enemy_feats_dim = self.get_state_ally_feats_size()
        ally_feats_dim = self.get_state_enemy_feats_size()
        own_feats_dim = self.get_state_own_feats_size_enemy()
        
        move_feats = np.zeros(move_feats_dim, dtype=np.float32)
        enemy_feats = np.zeros(enemy_feats_dim, dtype=np.float32)
        ally_feats = np.zeros(ally_feats_dim, dtype=np.float32)
        own_feats = np.zeros(own_feats_dim, dtype=np.float32)
        enemy_id_feats = np.zeros(self.n_enemies, dtype=np.float32)
        ...
        # Agent id features
        if self.state_agent_id:
            enemy_id_feats[enemy_id] = 1.
            state = np.append(state, enemy_id_feats.flatten())
    ...
    def get_global_state_enemy(self, enemy_id=-1):
        """
        Returns the global state.
        NOTE: This functon should not be used during decentralised execution.
        """
        if self.obs_instead_of_state:
            obs_concat = np.concatenate(self.get_enemy_obs(), axis=0).astype(np.float32)
            return obs_concat
        ...
        unit = self.get_enemy_by_id(enemy_id)# get the unit of some agent 
        x = unit.pos.x
        y = unit.pos.y
        sight_range = self.unit_sight_range(enemy_id)
        avail_actions = self.get_avail_enemy_actions(enemy_id) 
        ...
        if self.add_local_obs:
            state = np.append(state, self.get_obs_enemy(enemy_id).flatten())

        if self.state_timestep_number:
            state = np.append(state, self._episode_steps / self.episode_limit)

        if self.add_agent_id:
            agent_id_feats[enemy_id] = 1.0
            state = np.append(state, agent_id_feats.flatten())
        ...
        for al_id, al_unit in self.agents.items():
            if al_unit.health > 0:
                ...
                if self.stochastic_health:
                    ...
                    ind += 1
                if self.conic_fov:
                    ally_state[al_id, ind : ind + 2] = self.fov_directions_agent[
                        al_id
                    ]
                if self.unit_type_bits > 0:
                    type_id = self.get_unit_type_id(al_unit, True)
    ...
    def get_state_own_feats_size_enemy(self):
        """Returns the size of the vector containing the enemies' own features.
        """
        own_feats = 4 + self.unit_type_bits
        if self.obs_own_health:
    ...
    def get_obs_agent_feats_size(self):
        """
        Returns the dimensions of the matrix containing enemy(agent) features.
        Size is n_agents x n_features
        """
        nf_en = 4 + self.unit_type_bits
    ...
    def get_enemy_obs_size(self):
        """Returns the size of the observation."""
        own_feats = self.get_obs_own_feats_size_enemy()
        move_feats = self.get_obs_move_feats_size()
    ...
    def get_enemy_state_size(self):
        """Returns the size of the global state."""
        if self.obs_instead_of_state:
            return [self.get_enemy_obs_size()[0] * self.n_enemies, [self.n_enemies, self.get_enemy_obs_size()[0]]]

        if self.use_state_agent:
            own_feats = self.get_state_own_feats_size_enemy()
            move_feats = self.get_obs_move_feats_size()

            n_enemies, n_enemy_feats = self.get_state_ally_feats_size()
            n_allies, n_ally_feats = self.get_state_enemy_feats_size()
            ...
            return [all_feats * self.stacked_frames if self.use_stacked_frames else all_feats, 
                    [n_allies, n_ally_feats], [n_enemies, n_enemy_feats], 
                    [1, move_feats], [1, own_feats+agent_id_feats+timestep_feats]]
        ...
        enemy_state = self.n_agents * nf_en
        ally_state = self.n_enemies * nf_al
        ...
        if self.add_local_obs:
            obs_agent_size = self.get_enemy_obs_size()[0]
            size += obs_agent_size
        ...
        return [size * self.stacked_frames if self.use_stacked_frames else size, [self.n_enemies, nf_al], [self.n_agents, nf_en], [1, move_state + obs_agent_size + timestep_state + agent_id_feats]]
                ...
                if t_unit.health > 0:
                    if not self.action_mask:
                        ...
                        can_shoot = (
                            ...
                            else self.is_position_in_unit_cone(
    ...
    def get_avail_enemy_actions(self, enemy_id):
        """Returns the available actions for enemy_id."""
        unit = self.get_enemy_by_id(enemy_id)
        if unit.health > 0:
            # cannot choose no-op when alive
            ...
            # Can attack only alive units that are alive in the shooting range
            shoot_range = self.enemy_shoot_range(enemy_id)
                ...
                if t_unit.health > 0:
                    if not self.action_mask:
                        ...
                        can_shoot = (
                            dist <= shoot_range
                            if not self.conic_fov
                            else self.is_position_in_enemy_cone(
                                enemy_id, t_unit.pos, range="shoot_range"
                            )
                        )
    ...
    def get_agent_can_shoot(self, agent_id, t_unit):
        unit = self.get_unit_by_id(agent_id)
        dist = self.distance(
            unit.pos.x, unit.pos.y, t_unit.pos.x, t_unit.pos.y
        )
    ...
    def get_enemy_can_shoot(self, enemy_id, t_unit):
        unit = self.get_enemy_by_id(enemy_id)
        dist = self.distance(
            unit.pos.x, unit.pos.y, t_unit.pos.x, t_unit.pos.y
        )
    ...
    def get_true_avail_enemy_actions(self, enemy_id):
        """Returns the available actions for enemy_id."""
        unit = self.get_enemy_by_id(enemy_id)
        if unit.health > 0:
            # cannot choose no-op when alive
    ...
    def _kill_units(self, unit_tags, ally=True):
        debug_command = [
            d_pb.DebugCommand(kill_unit=d_pb.DebugKillUnit(tag=unit_tags))
        ]
        # index = self.agent_index if ally else self.enemy_index
        # self._parallel.run((c.debug, debug_command) for c in self._controllers)
        # self._controllers[index].debug(debug_command)
        self._controllers[self.agent_index].debug(debug_command)

    def _kill_all_units(self):
        """Kill all units on the map. Steps controller and so can throw
        exceptions"""
        ally_units = [unit.tag for unit in self._obs.observation.raw_data.units
                      if unit.owner == self.agent_team]
        enemy_units = [unit.tag for unit in self._controllers[self.enemy_index].observe().observation.raw_data.units
                      if unit.owner == self.enemy_team]
        dc_agent = [
            d_pb.DebugCommand(kill_unit=d_pb.DebugKillUnit(tag=ally_units))
        ]
        dc_enemy = [
            d_pb.DebugCommand(kill_unit=d_pb.DebugKillUnit(tag=enemy_units))
        ]
        # self._parallel.run((c.debug, dc) for c, dc in zip(self._controllers, [dc_agent, dc_enemy]))
        self._kill_units(ally_units, True)
        self._kill_units(enemy_units, False)
        # check the units are dead
        units = len(self._obs.observation.raw_data.units)
        while len(self._obs.observation.raw_data.units) > 0:
            a_units = len([u for u in self._obs.observation.raw_data.units if u.owner == self.agent_team])
            e_units = len([u for u in self._obs.observation.raw_data.units if u.owner == self.enemy_team])
            # logging.info(f"Units: {a_units} Enemies: {e_units}")
    ...
    def _create_new_team(self, team, episode_config, ally):
        ...
        for unit_id, unit in enumerate(team):
            ...
            owner = self.agent_team if ally else self.enemy_team
        ...
        # logging.info(f"Create Agent Request: {len(debug_command)}, owner: {owner}")
            
        controller_index = self.agent_index if ally else self.enemy_index
    ...
    def init_units(self, ally_team, enemy_team, episode_config={}):
        ...
        if ally_team and enemy_team:
            ...
            try:
                # self._parallel.run((self._controllers[self.agent_index].debug, team) for team in a_t)
                # self._parallel.run((self._controllers[self.enemy_index].debug, team) for team in e_t)
                # self._parallel.run((c.debug, team) for c, team in zip(self._controllers, [a_t, e_t]))
                self._parallel.run((c.step, 1) for c in self._controllers)
                self._obs = self._controllers[self.agent_index].observe()
            except (protocol.ProtocolError, protocol.ConnectionError):
                # logging.info("Full restart required in init_units()")
                self.full_restart()
        ...
        while True:
            ...
            self.agents = {}
            self.enemies = {}
            # TODO: Consider making this sorted stuff for both sides
            ally_units = [
                unit
            ...
            self._unit_types = [
                ...
                unit.unit_type
                for unit in enemy_units_sorted
            ]
            s += 1
            ...
            try:
                ...
                # logging.info("Full restart required in bottom init_units()")
                self.full_restart()
                self.reset(episode_config=episode_config)
    ...
    def _init_ally_unit_types(self, min_unit_type):
        ...
        if "10gen_" in self.map_name:
            num_rl_units = 9
            self._min_unit_type = (
                len(self._controllers[self.agent_index].data().units) - num_rl_units
            )
