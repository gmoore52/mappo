from .StarCraft2v2.wrapper_multiplayer import StarCraftCapabilityEnvWrapper # original smac v2 environment
import random
...
class SMACv2(StarCraftCapabilityEnvWrapper):
    def __init__(self, **kwargs):
        ...
        self.n_actions = self.env.n_actions_no_attack + self.env.n_enemies


        
        for i in range(self.env.n_agents):
            self.action_space.append(Discrete(self.env.n_agent_actions))
        ...
        for i in range(self.env.n_enemies):
            self.enemy_action_space.append(Discrete(self.env.n_enemy_actions))
            self.enemy_observation_space.append(self.env.get_enemy_obs_size())
            self.enemy_share_observation_space.append(self.env.get_enemy_state_size())
